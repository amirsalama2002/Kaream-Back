...
@section('content')
    ...
    <form action="{{ route('date.update', $date->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="author" class="form-control" value="{{ $date->author }}" required>
        </div>

        <div class="mb-3">
            <label class="form-label">Current Cover Image</label><br>
            @if($date->cover_image)
                <img src="{{ asset('storage/' . $date->cover_image) }}" width="120" class="mb-2">
            @else
                <p>No cover image</p>
            @endif
        ...
        <div class="mb-3">
            ...
            @if($date->pdf_file)
                <a href="{{ asset('storage/' . $date->pdf_file) }}" class="btn btn-outline-secondary btn-sm" target="_blank">View Current PDF</a>
            @else
                <p>No PDF uploaded</p>
            @endif
        ...
        <a href="{{ route('date.index') }}" class="btn btn-secondary">Cancel</a>
    </form>
@endsection
