    ...
    <form action="{{ route('novels.update', $novel->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            @if($novel->cover_image)
                <img src="{{ asset('storage/' . $novel->cover_image) }}" width="120" class="mb-2">
            @else
                <p>No cover image</p>
            @endif
        ...
        <div class="mb-3">
            ...
            @if($novel->pdf_file)
                <a href="{{ asset('storage/' . $novel->pdf_file) }}" class="btn btn-outline-secondary btn-sm" target="_blank">View Current PDF</a>
            @else
                <p>No PDF uploaded</p>
            @endif
